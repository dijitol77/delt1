...
void ProteusAudioProcessorEditor::resized()
{
    ...
    auto rowA1 = rowA.removeFromLeft(rowA.getWidth() / 3);
    auto rowA2 = rowA.removeFromLeft(rowA.getWidth() / 2);
    auto rowA3 = rowA;

    auto rowB1 = rowB.removeFromLeft(rowB.getWidth() / 3);
    auto rowB2 = rowB.removeFromLeft(rowB.getWidth() / 2);
    auto rowB3 = rowB;

    auto rowC1 = rowC.removeFromLeft(rowC.getWidth() / 3);
    auto rowC2 = rowC.removeFromLeft(rowC.getWidth() / 2);
    auto rowC3 = rowC;
    ...
    // Move odDriveKnob to the left by its own width and add a "margin" of 20 pixels to the left
    int marginLeftForKnob = 20;
    odDriveKnob.setBounds(rowA2.getX() + odDriveKnob.getWidth() + marginLeftForKnob, rowA2.getY(), odDriveKnob.getWidth(), odDriveKnob.getHeight());

    // Move cabOnButton down into rowC3 and add a "margin" of 20 pixels to the left
    int marginLeftForSwitch = 20;
    cabOnButton.setBounds(rowC3.getX() + rowC3.getWidth() / 2 - 25 + marginLeftForSwitch, rowC3.getY() + rowC3.getHeight() / 2 - 25, 50, 50);  // Explicit bounds in C3
