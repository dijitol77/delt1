...
ProteusAudioProcessor::ProteusAudioProcessor()
    ...
    cabSimIRa(BinaryData::default_ir_wav, BinaryData::default_ir_wavSize),
    LSTM1(),
    LSTM2(),
    LSTM3(),
    LSTM4(),
...
#endif
{
    ...
    eq4band.setParameters(bassValue, midValue, trebleValue, 0.0f);
    eq4band2.setParameters(bassValue, midValue, trebleValue, 0.0f);
    ...
    // Initialize state
    treeState.state = ValueTree("PARAMETERS");
}
...
void ProteusAudioProcessor::processBlock (AudioBuffer<float>& buffer, MidiBuffer& midiMessages)
{
    ...
    if (fw_state == 1 && model_loaded == true) {
        ...
        if (conditioned == false) {
            ...
            {
                buffer.applyGain(driveValue * 2.5f);
            }
            else {
                buffer.applyGainRamp(0, (int)buffer.getNumSamples(), previousDriveValue * 2.5f, driveValue * 2.5f);
            ...
            buffer.applyGain(1.5f); // Apply default boost to help sound
            ...
            for (int ch = 0; ch < buffer.getNumChannels(); ++ch)
            {
                ...
                if (ch == 0) {
                    ...
                    LSTM3.process(block44k.getChannelPointer(0), driveValue, block44k.getChannelPointer(0), (int)block44k.getNumSamples()); // Apply LSTM3 model
                }
                else if (ch == 1) {
                    LSTM2.process(block44k.getChannelPointer(1), driveValue, block44k.getChannelPointer(1), (int)block44k.getNumSamples());
                    LSTM4.process(block44k.getChannelPointer(1), driveValue, block44k.getChannelPointer(1), (int)block44k.getNumSamples()); // Apply LSTM4 model
                }
            }
            resampler.processOut(block44k, block);
        }

        // Add the processing for LSTM3 and LSTM4 (Assuming they have been initialized properly)
        // ... (processing with LSTM3 and LSTM4)
...
void ProteusAudioProcessor::loadConfig(File configFile)
{
    ...
    LSTM1.reset();
    LSTM2.reset();
    LSTM3.reset(); // Reset LSTM3 here
    LSTM4.reset(); // Reset LSTM4 here

    LSTM1.load_json(char_filename);
    LSTM2.load_json(char_filename);
    LSTM3.load_json(char_filename); // Load LSTM3 model here
    LSTM4.load_json(char_filename); // Load LSTM4 model here
...
AudioProcessorValueTreeState::ParameterLayout ProteusAudioProcessor::createParameterLayout()
{
    std::vector<std::unique_ptr<RangedAudioParameter>> parameters;
    parameters.push_back(std::make_unique<AudioParameterFloat>("ampBass", "Amp Bass", 0.0f, 1.0f, 0.5f));
    parameters.push_back(std::make_unique<AudioParameterFloat>("ampMid", "Amp Mid", 0.0f, 1.0f, 0.5f));
    parameters.push_back(std::make_unique<AudioParameterFloat>("ampTreble", "Amp Treble", 0.0f, 1.0f, 0.5f));
    parameters.push_back(std::make_unique<AudioParameterFloat>("odDrive", "OD Drive", 0.0f, 1.0f, 0.5f));
